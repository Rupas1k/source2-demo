...
impl ReaderMethods for Reader {
    ...
    fn remain_bits(&mut self) -> u32 {
        self.remain_bytes() + self.bit_count
    }
    ...
    fn read_bits(&mut self, amount: u32) -> u32 {
        while amount > self.bit_count {
            self.bit_val |= u64::from(self.read_next_byte()) << self.bit_count;
            self.bit_count += 8;
        }
        let bits = self.bit_val & ((1 << amount) - 1);
        self.bit_val >>= amount;
    ...
    fn read_bytes(&mut self, amount: u32) -> Vec<u8> {
        match self.bit_count {
            0 => {
                ...
                self.buf[(self.pos - amount) as usize..self.pos as usize].to_vec()
            }
            _ => {
    ...
    fn read_bits_as_bytes(&mut self, mut n: u32) -> Vec<u8> {
...
//         let breader = BigEndianReader::new(&bytes);
//         let mut reader = ReplayReader::new(breader);
//         // println!("{}", reader.reader.bits_remaining().unwrap());
//         let x = reader.read_bytes(1).unwrap()[0];
//
//         // assert_eq!(2, x);
//     }
// }
